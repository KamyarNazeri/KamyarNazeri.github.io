...
for l in np.arange(.1, 1, .1):
    ...
    with tf.Session() as sess:
        ...
        for j in range(1, 11):
            x_data, y_data = ds.make_moons(n_samples=n_samples, shuffle=True, noise=l)
    ...
    print(np.round(train_acc * 10, 2))
    train_accs.append(np.round(train_acc * 10, 2))
    test_accs.append(np.round(test_acc * 10, 2))
